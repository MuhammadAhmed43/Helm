...
TrackyMouse.init = function (div, { statsJs = false } = {}) {
		...
		<div class="tracky-mouse-controls">
			...
			<label class="tracky-mouse-control-row">
				...
				<span class="tracky-mouse-labeled-slider">
					...
					<span class="tracky-mouse-min-label">None</span>
					<span class="tracky-mouse-max-label">Max</span>
				</span>
			</label>
			<label class="tracky-mouse-control-row">
	...
	var smoothingSlider = uiContainer.querySelector(".tracky-mouse-smoothing");
	...
	function deserializeSettings(settings, initialLoad = false) {
		...
		if ("globalSettings" in settings) {
			...
			if (settings.globalSettings.headTrackingSmoothing !== undefined) {
				smoothing = settings.globalSettings.headTrackingSmoothing;
				smoothingSlider.value = smoothing * 100;
			}
	...
	function serializeSettings() {
		...
		return {
			...
			globalSettings: {
				...
				headTrackingSmoothing: smoothing,
				// TODO:
	...
	smoothingSlider.onchange = (event) => {
		smoothing = smoothingSlider.value / 100;
		if (event) {
			setOptions({ globalSettings: { headTrackingSmoothing: smoothing } });
		}
	};
	...
	function draw(update = true) {
		...
		if (update) {
			...
			var targetDeltaX = accelerate(movementX * sensitivityX, distance);
			var targetDeltaY = accelerate(movementY * sensitivityY, distance);

			var alpha = 1 - (smoothing * 0.9);
			smoothedDeltaX = (smoothedDeltaX || 0) * (1 - alpha) + targetDeltaX * alpha;
			smoothedDeltaY = (smoothedDeltaY || 0) * (1 - alpha) + targetDeltaY * alpha;

			var deltaX = smoothedDeltaX;
			var deltaY = smoothedDeltaY;
