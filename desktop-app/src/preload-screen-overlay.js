...
contextBridge.exposeInMainWorld('electronAPI', {
	...
	mouseClick: (x, y, button) => ipcRenderer.send('click', x, y, button, performance.now()),
	mouseDown: (x, y, button) => ipcRenderer.send('mouse-down', x, y, button, performance.now()),
	mouseUp: (x, y, button) => ipcRenderer.send('mouse-up', x, y, button, performance.now()),
	mouseScroll: (direction) => ipcRenderer.send('scroll', direction, performance.now()),
	keyboardType: (text) => ipcRenderer.send('keyboard-type', text),
	keyboardPress: (key) => ipcRenderer.send('keyboard-press', key, performance.now()),
});
