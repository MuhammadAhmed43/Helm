...
const { app, globalShortcut, dialog, BrowserWindow, ipcMain, screen } = require('electron');
...
const { setMouseLocation: setMouseLocationWithoutTracking, getMouseLocation, click, mouseDown, mouseUp, typeText, pressKey } = require('serenade-driver');
...
async function setMouseLocationTracky(x, y) {
	...
	const screenPoint = screen.dipToScreenPoint({ x: Math.round(x), y: Math.round(y) });
	await setMouseLocationWithoutTracking(screenPoint.x, screenPoint.y);
}
...
const createWindow = () => {
	...
	ipcMain.on('move-mouse', async (_event, x, y, time) => {
		...
		const curPosPhysical = await getMouseLocation();
		const curPos = screen.screenToDipPoint(curPosPhysical);
	...
	ipcMain.on('notify-toggle-state', async (_event, nowEnabled) => {
		...
		if (nowEnabled) { // don't rely on getMouseLocation when disabling the software
			const initialPosPhysical = await getMouseLocation();
			initialPos = screen.screenToDipPoint(initialPosPhysical);
		}
		enabled = nowEnabled;
		updateDwellClicking();
	...
	ipcMain.on('click', async (_event, x, y, button, _time) => {
		...
		await click(button || (swapMouseButtons ? "right" : "left"));

		// const latency = performance.now() - time;
		// console.log(`click: ${x}, ${y}, latency: ${latency}`);
	});

	ipcMain.on('mouse-down', async (_event, x, y, button, _time) => {
		if (regainControlTimeout || !enabled) {
			return;
		}
		if (
		...
		y += screenOverlayWindow.getContentBounds().y;
		await setMouseLocationTracky(x, y);
		await mouseDown(button || (swapMouseButtons ? "right" : "left"));
	});

	ipcMain.on('mouse-up', async (_event, x, y, button, _time) => {
		if (
			(!screenOverlayWindow || screenOverlayWindow.isDestroyed()) ||
			(!appWindow || appWindow.isDestroyed())
		) {
		...
		y += screenOverlayWindow.getContentBounds().y;
		await setMouseLocationTracky(x, y);
		await mouseUp(button || (swapMouseButtons ? "right" : "left"));
	});

	ipcMain.on('scroll', (_event, direction, _time) => {
		if (regainControlTimeout || !enabled) {
			return;
		}
		// Use PageUp/PageDown for reliable scrolling in browsers/YouTube
		const key = direction === "up" ? "pageup" : "pagedown";
		pressKey(key);
	});

	ipcMain.on('keyboard-type', (_event, text) => {
		typeText(text);
	});

	ipcMain.on('keyboard-press', (_event, key) => {
		pressKey(key);
	});
	...
	screenOverlayWindow = new BrowserWindow({
		...
		focusable: false,
		skipTaskbar: true,
		accessibleTitle: 'Helm Screen Overlay',
		webPreferences: {
