...
const aboutItem = {
	...
	click: async () => {
		...
		openAboutWindow({
			...
			description: 'Helm — steer your cursor with your head.',
			license: 'MIT',
			product_name: 'Helm',
		});
	},
		...
		submenu: [
			...
			{ role: 'toggleDevTools' },
			{
				...
				click: async () => {
					...
					const screenOverlayWindow = BrowserWindow.getAllWindows().find(window => window.getTitle() === 'Helm Screen Overlay');
